...
import { type ProductListItem, ProductsListItem } from "@/features/products";
...
const { pending, execute } = useAsyncState(() => cartApplication.useRemoveCartItem()(props.id))
</script>

<template>
  <div class="relative">
    <ProductsListItem v-bind="props" />

    <button
      type="button"
      class="absolute top-4 right-4 p-1 rounded-full bg-white w-10 h-10 z-10"
      :disabled="pending"
      @click="execute"
    >
      <svg
